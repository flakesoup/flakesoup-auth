package com.flakesoup.auth.auth2.config.exception;

import com.flakesoup.auth.exception.*;
...
public class OAuth2WebResponseExceptionTranslator implements WebResponseExceptionTranslator<OAuth2Exception> {
	...
	private ResponseEntity<OAuth2Exception> handleOAuth2Exception(OAuth2Exception e) {
		...
		return new ResponseEntity<>(new BaseAuthException(e.getMessage(), e.getOAuth2ErrorCode()), headers,
			HttpStatus.valueOf(status));
